...
export const { auth, handlers, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    async jwt({ token, user, account }) {
      ...
      // Check for state in account.id_token and set redirect if present
      if (account && account.id_token) {
        try {
          const idToken = JSON.parse(Buffer.from(account.id_token.split('.')[1], 'base64').toString());
          if (idToken.state && idToken.state.origin) {
            token.redirect = idToken.state.origin;
          }
        } catch (error) {
    ...
    async redirect({ url, baseUrl, session }) {
      ...
      // Use redirect information from session if available
      if (session?.redirect) {
