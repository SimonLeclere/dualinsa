...
export default function ReviewLesson({ reviewLessonShown, setReviewLessonShown, questionResults }) {
  ...
  return (
    <div className={`fixed inset-0 flex items-center justify-center p-5 transition duration-300 ${reviewLessonShown ? "" : "pointer-events-none opacity-0"}`}>
      <div
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {questionResults.map((questionResult, i) => {
            return (
              <div
                key={i}
                className={`relative flex flex-col items-stretch hyphens-auto break-all gap-3 rounded-xl p-5 text-left ${questionResult.isCorrect ? "bg-green-100 text-green-600" : "bg-red-100 text-red-500"}`}
              >
                <div className="flex justify-between gap-2 break-normal">
                  <h3 className="font-bold">{questionResult.question}</h3>
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white">
                    {questionResult.isCorrect ? <DoneSvg className="h-5 w-5" /> : <BigCloseSvg className="h-5 w-5" />}
                  </div>
                </div>
                {
                  questionResult.isCorrect
                    ? <div className="text-green-600">{questionResult.userAnswer}</div>
                    : (
                      <>
                        <div className="text-red-500 line-through">{questionResult.userAnswer || "Aucune réponse"}</div>
                        <div className="text-sm font-normal">{questionResult.correctAnswer}</div>
                      </>
                    )
                }
              </div>
